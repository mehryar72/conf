...
parser.add_argument("--BS", type=int, default=6)  # 3 modeR==1 9 if modeR==0
...
name = '/scratch/mabbasib/MLLOG/log2_' + (''.join(sys.argv[1:]))
if args.cont:
    logF = name+'con' + '.csv'
    netS2 = name + '.pth'
    netS = name+'con' + '.pth'
...
if __name__ == '__main__':
    ...
    D_T, D_V, D_Te = datPP3(root_dir='./grab_cropped/')
    DT_loader = MakeH2(D_T,transform=transform)
    DV_loader = MakeH2(D_V,transform=transformV)
    DTe_loader = MakeH2(D_Te,transform=transformV)
            ...
            for data in data_loaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        Scorelist.append(torch.cat([labels.float().cpu().unsqueeze(dim=1), outputs.cpu().float(),torch.max(outputs.data, 1)[1].cpu().unsqueeze(1).float(),data[2].unsqueeze(1).float(),data[3].unsqueeze(1).float(),data[4].unsqueeze(1).float(),data[5].unsqueeze(1).float()], dim=1))
